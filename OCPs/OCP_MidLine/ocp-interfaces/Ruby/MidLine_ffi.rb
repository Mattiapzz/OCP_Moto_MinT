...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libMidLine.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libMidLine.#{DYLIB_EXT}"
end
...
module MidLine
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libMidLine.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = MidLine.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = MidLine.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = MidLine.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      MidLine.OCP_write_ocp_solution( @id, fname )
    end
