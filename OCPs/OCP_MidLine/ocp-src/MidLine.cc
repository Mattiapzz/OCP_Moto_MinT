...
namespace MidLineDefine {
  ...
  */
  void
  MidLine::updateContinuation( integer phase, real_type s ) {
    LW_ASSERT(
      s >= 0 && s <= 1,
      "MidLine::updateContinuation( phase number = {}, s = {}) "
  ...
  */
  void
  MidLine::setupParameters( GenericContainer const & gc_data ) {
    LW_ASSERT0(
      gc_data.exists("Parameters"),
      "MidLine::setupParameters: Missing key `Parameters` in data\n"
    );
    ...
    LW_ASSERT0(
      allfound, "in MidLine::setup not all parameters are set!\n"
    );
  }

  void
  MidLine::setupParameters( real_type const Pars[] ) {
  ...
  */
  void
  MidLine::setupUserClasses( GenericContainer const & gc ) {
  }
  ...
  */
  void
  MidLine::setupUserMappedFunctions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  MidLine::setupControls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    LW_ASSERT0(
      gc_data.exists("Controls"),
      "MidLine::setupClasses: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  MidLine::setupPointers( GenericContainer const & gc_data ) {

    LW_ASSERT0(
      gc_data.exists("Pointers"),
      "MidLine::setupPointers: Missing key `Pointers` in data\n"
    );
    ...
    LW_ASSERT0(
      gc.exists("pEngine"),
      "in MidLine::setupPointers(gc) cant find key `pEngine' in gc\n"
    );
    pEngine = gc("pEngine").get_pointer<Engine*>();

    LW_ASSERT0(
      gc.exists("pRoad"),
      "in MidLine::setupPointers(gc) cant find key `pRoad' in gc\n"
    );
  ...
  */
  void
  MidLine::setup( GenericContainer const & gc ) {
  ...
  */
  void
  MidLine::get_names( GenericContainer & out ) const {
  ...
  */
  void
  MidLine::diagnostic( GenericContainer const & gc ) {
  ...
  void
  MidLine::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
