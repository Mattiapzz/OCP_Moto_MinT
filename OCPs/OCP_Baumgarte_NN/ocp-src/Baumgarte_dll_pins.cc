...
#define MAP_PROBLEM map< string, Baumgarte_Problem * >
...
namespace BaumgarteDefine {
  ...
  BAUMGARTE_API_DLL
  mrb_value
  mrb_Baumgarte_ocp_setup( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Baumgarte_ocp_setup, missing first parameter or is nota a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Baumgarte_ocp_setup, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      problems[id] = new Baumgarte_Problem( pTP, pConsole );
  ...
  BAUMGARTE_API_DLL
  mrb_value
  mrb_Baumgarte_ocp_guess( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Baumgarte_ocp_guess, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Baumgarte_ocp_guess, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Baumgarte_ocp_guess, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      ...
      it->second->guess(
        gc_data(
          "Guess",
          "mrb_Baumgarte_ocp_guess Missing field `Guess`"
        )
      );
    }
  ...
  BAUMGARTE_API_DLL
  mrb_value
  mrb_Baumgarte_ocp_solve( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Baumgarte_ocp_solve, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Baumgarte_ocp_solve, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Baumgarte_ocp_solve, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      ...
      if ( !ok )
        pConsole->error( "Baumgarte_ocp_solve, solution failed!\n" );
  ...
  BAUMGARTE_API_DLL
  mrb_value
  mrb_Baumgarte_write_ocp_solution( mrb_state *mrb, mrb_value self ) {
    mrb_value a_string;
    ...
    MAP_PROBLEM::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error( "Baumgarte_ocp_write_to_file, no solution found!\n" );
    } else {
  ...
  BAUMGARTE_API_DLL
  mrb_value
  mrb_Baumgarte_ocp_solution_range( mrb_state *mrb, mrb_value self ) {
  ...
  BAUMGARTE_API_DLL
  mrb_value
  mrb_Baumgarte_ocp_eval_solution( mrb_state *mrb, mrb_value self ) {
  ...
  BAUMGARTE_API_DLL
  void
  mrb_libBaumgarte_gem_init(mrb_state* mrb) {
    ...
    mrb_define_const(
      ...
      ocp_class,
      "NAME",
      mrb_str_new_cstr(mrb, "Baumgarte")
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "write_ocp_solution",
      mrb_Baumgarte_write_ocp_solution,
      MRB_ARGS_REQ(1)
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "eval_solution",
      mrb_Baumgarte_ocp_eval_solution,
      MRB_ARGS_REQ(2)
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "solution_range",
      mrb_Baumgarte_ocp_solution_range,
      MRB_ARGS_REQ(1)
    );
  ...
  BAUMGARTE_API_DLL
  void
  mrb_libBaumgarte_gem_final(mrb_state* mrb)
  {
  }
}

// EOF: Baumgarte_dll_pins.cc
