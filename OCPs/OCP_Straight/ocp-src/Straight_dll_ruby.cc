...
namespace StraightDefine {
  ...
  static map< string, Straight_Problem * > problems;
  ...
  */
  ...
  Straight_ocp_setup( char const id[], GenericContainer & gc_data ) {
    ...
    map< string, Straight_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  STRAIGHT_API_DLL
  bool
  Straight_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, Straight_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "Straight_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  Straight_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("Straight_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  Straight_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, Straight_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("Straight_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
