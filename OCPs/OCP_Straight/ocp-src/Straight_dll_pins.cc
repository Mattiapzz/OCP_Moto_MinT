...
#define MAP_PROBLEM map< string, Straight_Problem * >
...
namespace StraightDefine {
  ...
  STRAIGHT_API_DLL
  mrb_value
  mrb_Straight_ocp_setup( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Straight_ocp_setup, missing first parameter or is nota a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Straight_ocp_setup, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      problems[id] = new Straight_Problem( pTP, pConsole );
  ...
  STRAIGHT_API_DLL
  mrb_value
  mrb_Straight_ocp_guess( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Straight_ocp_guess, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Straight_ocp_guess, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Straight_ocp_guess, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      ...
      it->second->guess(
        gc_data(
          "Guess",
          "mrb_Straight_ocp_guess Missing field `Guess`"
        )
      );
    }
  ...
  STRAIGHT_API_DLL
  mrb_value
  mrb_Straight_ocp_solve( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Straight_ocp_solve, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Straight_ocp_solve, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Straight_ocp_solve, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      ...
      if ( !ok )
        pConsole->error( "Straight_ocp_solve, solution failed!\n" );
  ...
  STRAIGHT_API_DLL
  mrb_value
  mrb_Straight_write_ocp_solution( mrb_state *mrb, mrb_value self ) {
    mrb_value a_string;
    ...
    MAP_PROBLEM::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error( "Straight_ocp_write_to_file, no solution found!\n" );
    } else {
  ...
  STRAIGHT_API_DLL
  mrb_value
  mrb_Straight_ocp_solution_range( mrb_state *mrb, mrb_value self ) {
  ...
  STRAIGHT_API_DLL
  mrb_value
  mrb_Straight_ocp_eval_solution( mrb_state *mrb, mrb_value self ) {
  ...
  STRAIGHT_API_DLL
  void
  mrb_libStraight_gem_init(mrb_state* mrb) {
    ...
    mrb_define_method(
      ...
      ocp_class,
      "write_ocp_solution",
      mrb_Straight_write_ocp_solution,
      MRB_ARGS_REQ(1)
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "eval_solution",
      mrb_Straight_ocp_eval_solution,
      MRB_ARGS_REQ(2)
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "solution_range",
      mrb_Straight_ocp_solution_range,
      MRB_ARGS_REQ(1)
    );
  ...
  STRAIGHT_API_DLL
  void
  mrb_libStraight_gem_final(mrb_state* mrb)
  {
  }
}

// EOF: Straight_dll_pins.cc
