...
namespace StraightDefine {
  ...
  */
  void
  Straight::updateContinuation( integer phase, real_type s ) {
    LW_ASSERT(
      s >= 0 && s <= 1,
      "Straight::updateContinuation( phase number = {}, s = {}) "
  ...
  */
  void
  Straight::setupParameters( GenericContainer const & gc_data ) {
    LW_ASSERT0(
      gc_data.exists("Parameters"),
      "Straight::setupParameters: Missing key `Parameters` in data\n"
    );
    ...
    LW_ASSERT0(
      allfound, "in Straight::setup not all parameters are set!\n"
    );
  }

  void
  Straight::setupParameters( real_type const Pars[] ) {
  ...
  */
  void
  Straight::setupUserClasses( GenericContainer const & gc ) {
  }
  ...
  */
  void
  Straight::setupUserMappedFunctions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  Straight::setupControls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    LW_ASSERT0(
      gc_data.exists("Controls"),
      "Straight::setupClasses: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  Straight::setupPointers( GenericContainer const & gc_data ) {

    LW_ASSERT0(
      gc_data.exists("Pointers"),
      "Straight::setupPointers: Missing key `Pointers` in data\n"
    );
    ...
    LW_ASSERT0(
      gc.exists("pEngine"),
      "in Straight::setupPointers(gc) cant find key `pEngine' in gc\n"
    );
    pEngine = gc("pEngine").get_pointer<Engine*>();

    LW_ASSERT0(
      gc.exists("pRoad"),
      "in Straight::setupPointers(gc) cant find key `pRoad' in gc\n"
    );
  ...
  */
  void
  Straight::setup( GenericContainer const & gc ) {
  ...
  */
  void
  Straight::get_names( GenericContainer & out ) const {
  ...
  */
  void
  Straight::diagnostic( GenericContainer const & gc ) {
  ...
  void
  Straight::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
