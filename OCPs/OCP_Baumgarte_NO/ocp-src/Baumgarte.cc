...
namespace BaumgarteDefine {
  ...
  */
  void
  Baumgarte::updateContinuation( integer phase, real_type s ) {
    LW_ASSERT(
      s >= 0 && s <= 1,
      "Baumgarte::updateContinuation( phase number = {}, s = {}) "
    ...
    switch ( phase ) {
      ...
      case 4: continuationStep4( s ); break;
      default:
        LW_ERROR(
          "Baumgarte::updateContinuation( phase number = {}, s = {} )"
  ...
  */
  void
  Baumgarte::setupParameters( GenericContainer const & gc_data ) {
    LW_ASSERT0(
      gc_data.exists("Parameters"),
      "Baumgarte::setupParameters: Missing key `Parameters` in data\n"
    );
    ...
    LW_ASSERT0(
      allfound, "in Baumgarte::setup not all parameters are set!\n"
    );
  }

  void
  Baumgarte::setupParameters( real_type const Pars[] ) {
  ...
  */
  void
  Baumgarte::setupClasses( GenericContainer const & gc_data ) {
    LW_ASSERT0(
      gc_data.exists("Constraints"),
      "Baumgarte::setupClasses: Missing key `Parameters` in data\n"
    );
    GenericContainer const & gc = gc_data("Constraints");
    // Initialize Constraints 1D
    LW_ASSERT0(
      gc.exists("FrontWheelContact"),
      "in Baumgarte::setupClasses(gc) missing key: ``FrontWheelContact''\n"
    );
    FrontWheelContact.setup( gc("FrontWheelContact") );

    LW_ASSERT0(
      gc.exists("RearWheelContact"),
      "in Baumgarte::setupClasses(gc) missing key: ``RearWheelContact''\n"
    );
    RearWheelContact.setup( gc("RearWheelContact") );

    LW_ASSERT0(
      gc.exists("LongSlipFront"),
      "in Baumgarte::setupClasses(gc) missing key: ``LongSlipFront''\n"
    );
    LongSlipFront.setup( gc("LongSlipFront") );

    LW_ASSERT0(
      gc.exists("LongSlipRear"),
      "in Baumgarte::setupClasses(gc) missing key: ``LongSlipRear''\n"
    );
    LongSlipRear.setup( gc("LongSlipRear") );

    LW_ASSERT0(
      gc.exists("LatSlipFront"),
      "in Baumgarte::setupClasses(gc) missing key: ``LatSlipFront''\n"
    );
    LatSlipFront.setup( gc("LatSlipFront") );

    LW_ASSERT0(
      gc.exists("LatSlipRear"),
      "in Baumgarte::setupClasses(gc) missing key: ``LatSlipRear''\n"
    );
    LatSlipRear.setup( gc("LatSlipRear") );

    LW_ASSERT0(
      gc.exists("MaxBetaAngle"),
      "in Baumgarte::setupClasses(gc) missing key: ``MaxBetaAngle''\n"
    );
    MaxBetaAngle.setup( gc("MaxBetaAngle") );

    LW_ASSERT0(
      gc.exists("MaxSteerAngle"),
      "in Baumgarte::setupClasses(gc) missing key: ``MaxSteerAngle''\n"
    );
    MaxSteerAngle.setup( gc("MaxSteerAngle") );

    LW_ASSERT0(
      gc.exists("MaxRollAngle"),
      "in Baumgarte::setupClasses(gc) missing key: ``MaxRollAngle''\n"
    );
    MaxRollAngle.setup( gc("MaxRollAngle") );

    LW_ASSERT0(
      gc.exists("roadRightLateralBorder"),
      "in Baumgarte::setupClasses(gc) missing key: ``roadRightLateralBorder''\n"
    );
    roadRightLateralBorder.setup( gc("roadRightLateralBorder") );

    LW_ASSERT0(
      gc.exists("roadLeftLateralBorder"),
      "in Baumgarte::setupClasses(gc) missing key: ``roadLeftLateralBorder''\n"
    );
  ...
  */
  void
  Baumgarte::setupUserClasses( GenericContainer const & gc ) {
  }
  ...
  */
  void
  Baumgarte::setupUserMappedFunctions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  Baumgarte::setupControls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    LW_ASSERT0(
      gc_data.exists("Controls"),
      "Baumgarte::setupClasses: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  Baumgarte::setupPointers( GenericContainer const & gc_data ) {

    LW_ASSERT0(
      gc_data.exists("Pointers"),
      "Baumgarte::setupPointers: Missing key `Pointers` in data\n"
    );
    ...
    LW_ASSERT0(
      gc.exists("pEngine"),
      "in Baumgarte::setupPointers(gc) cant find key `pEngine' in gc\n"
    );
    pEngine = gc("pEngine").get_pointer<Engine*>();

    LW_ASSERT0(
      gc.exists("pRoad"),
      "in Baumgarte::setupPointers(gc) cant find key `pRoad' in gc\n"
    );
  ...
  */
  ...
  Baumgarte::infoClasses() const {
    ...
    pConsole->message(mstr.str(),msg_level);
    mstr.str(""); MaxBetaAngle          .info(mstr);
    pConsole->message(mstr.str(),msg_level);
  ...
  */
  void
  Baumgarte::setup( GenericContainer const & gc ) {
  ...
  */
  void
  Baumgarte::get_names( GenericContainer & out ) const {
  ...
  */
  void
  Baumgarte::diagnostic( GenericContainer const & gc ) {
  ...
  void
  Baumgarte::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
