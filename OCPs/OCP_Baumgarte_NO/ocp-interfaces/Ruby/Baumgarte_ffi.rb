...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libBaumgarte.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libBaumgarte.#{DYLIB_EXT}"
end
...
module Baumgarte
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libBaumgarte.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = Baumgarte.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = Baumgarte.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = Baumgarte.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      Baumgarte.OCP_write_ocp_solution( @id, fname )
    end
