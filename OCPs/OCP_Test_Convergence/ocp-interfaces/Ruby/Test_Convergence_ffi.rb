...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libTest_Convergence.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libTest_Convergence.#{DYLIB_EXT}"
end
...
module Test_Convergence
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libTest_Convergence.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = Test_Convergence.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = Test_Convergence.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = Test_Convergence.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      Test_Convergence.OCP_write_ocp_solution( @id, fname )
    end
