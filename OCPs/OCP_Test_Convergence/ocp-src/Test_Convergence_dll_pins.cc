/*-----------------------------------------------------------------------*\
 |  file: Test_Convergence_dll_pins.cc                                   |
 |                                                                       |
...
#include "Test_Convergence_dll_pins.hh"
#include <map>
...
#define MAP_PROBLEM map< string, Test_Convergence_Problem * >
...
namespace Test_ConvergenceDefine {
  ...
  mrb_Test_Convergence_ocp_setup( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Test_Convergence_ocp_setup, missing first parameter or is nota a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Test_Convergence_ocp_setup, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      problems[id] = new Test_Convergence_Problem( pTP, pConsole );
  ...
  mrb_Test_Convergence_ocp_guess( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Test_Convergence_ocp_guess, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Test_Convergence_ocp_guess, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Test_Convergence_ocp_guess, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      ...
      it->second->guess(
        gc_data(
          "Guess",
          "mrb_Test_Convergence_ocp_guess Missing field `Guess`"
        )
      );
    }
  ...
  mrb_Test_Convergence_ocp_solve( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Test_Convergence_ocp_solve, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Test_Convergence_ocp_solve, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "Test_Convergence_ocp_solve, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      ...
      if ( !ok )
        pConsole->error( "Test_Convergence_ocp_solve, solution failed!\n" );
  ...
  mrb_Test_Convergence_write_ocp_solution( mrb_state *mrb, mrb_value self ) {
    ...
    MAP_PROBLEM::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error( "Test_Convergence_ocp_write_to_file, no solution found!\n" );
    } else {
  ...
  mrb_libTest_Convergence_gem_init(mrb_state* mrb) {
    ...
    mrb_define_const(
      ...
      ocp_class,
      "NAME",
      mrb_str_new_cstr(mrb, "Test_Convergence")
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "setup",
      mrb_Test_Convergence_ocp_setup,
      MRB_ARGS_NONE()
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "guess",
      mrb_Test_Convergence_ocp_guess,
      MRB_ARGS_NONE()
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "solve",
      mrb_Test_Convergence_ocp_solve,
      MRB_ARGS_NONE()
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "write_ocp_solution",
      mrb_Test_Convergence_write_ocp_solution,
      MRB_ARGS_REQ(1)
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "eval_solution",
      mrb_Test_Convergence_ocp_eval_solution,
      MRB_ARGS_REQ(2)
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "solution_range",
      mrb_Test_Convergence_ocp_solution_range,
      MRB_ARGS_REQ(1)
    );
...
// EOF: Test_Convergence_dll_pins.cc
