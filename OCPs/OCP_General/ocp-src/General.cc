...
namespace GeneralDefine {
  ...
  */
  void
  General::updateContinuation( integer phase, real_type s ) {
    LW_ASSERT(
      s >= 0 && s <= 1,
      "General::updateContinuation( phase number = {}, s = {}) "
  ...
  */
  void
  General::setupParameters( GenericContainer const & gc_data ) {
    LW_ASSERT0(
      gc_data.exists("Parameters"),
      "General::setupParameters: Missing key `Parameters` in data\n"
    );
    ...
    LW_ASSERT0(
      allfound, "in General::setup not all parameters are set!\n"
    );
  }

  void
  General::setupParameters( real_type const Pars[] ) {
  ...
  */
  void
  General::setupUserClasses( GenericContainer const & gc ) {
  }
  ...
  */
  void
  General::setupUserMappedFunctions( GenericContainer const & gc_data ) {
  }
  /* --------------------------------------------------------------------------
  //            _                ____            _             _
  ...
  */
  void
  General::setupControls( GenericContainer const & gc_data ) {
    // initialize Control penalties
    LW_ASSERT0(
      gc_data.exists("Controls"),
      "General::setupClasses: Missing key `Controls` in data\n"
    );
  ...
  */
  void
  General::setupPointers( GenericContainer const & gc_data ) {

    LW_ASSERT0(
      gc_data.exists("Pointers"),
      "General::setupPointers: Missing key `Pointers` in data\n"
    );
    ...
    LW_ASSERT0(
      gc.exists("pEngine"),
      "in General::setupPointers(gc) cant find key `pEngine' in gc\n"
    );
    pEngine = gc("pEngine").get_pointer<Engine*>();

    LW_ASSERT0(
      gc.exists("pRoad"),
      "in General::setupPointers(gc) cant find key `pRoad' in gc\n"
    );
  ...
  */
  void
  General::setup( GenericContainer const & gc ) {
  ...
  */
  void
  General::get_names( GenericContainer & out ) const {
  ...
  */
  void
  General::diagnostic( GenericContainer const & gc ) {
  ...
  void
  General::save_OCP_info( GenericContainer & gc ) const {
    for ( integer i = 0; i < numModelPars; ++i )
