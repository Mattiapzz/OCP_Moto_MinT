...
namespace GeneralDefine {
  ...
  static map< string, General_Problem * > problems;
  ...
  */
  ...
  General_ocp_setup( char const id[], GenericContainer & gc_data ) {
    ...
    map< string, General_Problem * >::iterator it = problems.find(id);
    string error;
    integer infoLevel = 1;
  ...
  GENERAL_API_DLL
  bool
  General_ocp_guess( char const id[], GenericContainer & gc_data ) {
    map< string, General_Problem * >::iterator it = problems.find(id);
    if ( it == problems.end() ) {
      return false;
    } else {
      it->second->guess( gc_data("Guess", "General_ocp_guess missing field `Guess`" ) );
      return true;
    }
  }
  ...
  General_ocp_solve( char const id[],
    ...
    if ( it == problems.end() ) {
      ...
      if ( ok ) {
        ...
        pConsole->error(
          fmt::format("General_ocp_solve, ERROR: {}\n",errmsg)
        );
      }
      return ok;
    }
  ...
  General_write_ocp_solution( char const id[], char const fname[] ) {
    map< string, General_Problem * >::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error("General_write_ocp_solution, no solution found!\n" );
    } else {
      ofstream file( fname );
