...
#define MAP_PROBLEM map< string, General_Problem * >
...
namespace GeneralDefine {
  ...
  GENERAL_API_DLL
  mrb_value
  mrb_General_ocp_setup( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "General_ocp_setup, missing first parameter or is nota a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "General_ocp_setup, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      problems[id] = new General_Problem( pTP, pConsole );
  ...
  GENERAL_API_DLL
  mrb_value
  mrb_General_ocp_guess( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "General_ocp_guess, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "General_ocp_guess, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "General_ocp_guess, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      ...
      it->second->guess(
        gc_data(
          "Guess",
          "mrb_General_ocp_guess Missing field `Guess`"
        )
      );
    }
  ...
  GENERAL_API_DLL
  mrb_value
  mrb_General_ocp_solve( mrb_state *mrb, mrb_value self ) {
    ...
    if ( !mrb_string_p(id_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "General_ocp_solve, missing first parameter or is not a string (id)\n"
      );
    }
    if ( mrb_nil_p(data_value) )  {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "General_ocp_solve, missing second parameter (data)\n"
      );
    }
    ...
    if ( it == problems.end() ) {
      mrb_raise(
        mrb,
        E_OCP_ERROR,
        "General_ocp_solve, you must do setup first!\n"
      );
    } else {
      // to C++ parameters
      GenericContainer gc_data;
      ...
      if ( !ok )
        pConsole->error( "General_ocp_solve, solution failed!\n" );
  ...
  GENERAL_API_DLL
  mrb_value
  mrb_General_write_ocp_solution( mrb_state *mrb, mrb_value self ) {
    mrb_value a_string;
    ...
    MAP_PROBLEM::const_iterator it = problems.find(id);
    if ( it == problems.end() ) {
      pConsole->error( "General_ocp_write_to_file, no solution found!\n" );
    } else {
  ...
  GENERAL_API_DLL
  mrb_value
  mrb_General_ocp_solution_range( mrb_state *mrb, mrb_value self ) {
  ...
  GENERAL_API_DLL
  mrb_value
  mrb_General_ocp_eval_solution( mrb_state *mrb, mrb_value self ) {
  ...
  GENERAL_API_DLL
  void
  mrb_libGeneral_gem_init(mrb_state* mrb) {
    ...
    mrb_define_method(
      ...
      ocp_class,
      "write_ocp_solution",
      mrb_General_write_ocp_solution,
      MRB_ARGS_REQ(1)
    );
    ...
    mrb_define_method(
      ...
      ocp_class,
      "solution_range",
      mrb_General_ocp_solution_range,
      MRB_ARGS_REQ(1)
    );
  ...
  GENERAL_API_DLL
  void
  mrb_libGeneral_gem_final(mrb_state* mrb)
  {
  }
}

// EOF: General_dll_pins.cc
