...
module GenericContainer
  @libGenericContainer = MECHATRONIX_LIB_PATH+"/libGeneral.#{DYLIB_EXT}"
end

module Splines
  @libSplines = MECHATRONIX_LIB_PATH+"/libGeneral.#{DYLIB_EXT}"
end
...
module General
  ...
  LIB_PATH = MECHATRONIX_LIB_PATH+"/libGeneral.#{DYLIB_EXT}"
  ...
  class OCPSolver
    ...
    def solve( convert = true )
      ...
      ok = General.OCP_setup( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok

      ok = General.OCP_guess( @id, GenericContainer.GC_mem_ptr(@id) )
      raise RuntimeError, "Failed to do guess: #{out[:Error]}\n" unless ok

      ok  = General.OCP_solve( @id,
    ...
    def write_ocp_solution( fname )
      General.OCP_write_ocp_solution( @id, fname )
    end
